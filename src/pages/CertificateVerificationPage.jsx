  ...
  return (
    <main className="certificate-verification-page">
      <section className="certificate-verification-card">
        <p className="verification-mark">RHOPEE / NEF</p>
        ...
        {status === 'verified' && <><span className="verified-label">LICENSED RHOPEE CERTIFICATE</span><h1>Certificate verified</h1><p>This certificate was issued by RHOPEE / NEF and published by an authorized administrator.</p><dl><div><dt>Holder</dt><dd>{result.certificate_name || result.student_name}</dd></div><div><dt>Certificate number</dt><dd>{result.certificate_number}</dd></div><div><dt>Course</dt><dd>{result.certificate_course}</dd></div><div><dt>Issue date</dt><dd>{result.certificate_issue_date}</dd></div></dl></>}
      </section>
    </main>
  );
}
